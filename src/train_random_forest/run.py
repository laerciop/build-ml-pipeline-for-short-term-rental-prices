...
def go(args):
    ...
    model_save_path = os.path.join("random_forest_dir", args.output_artifact)
    mlflow.sklearn.save_model(sk_pipe, model_save_path)

    model_export = wandb.Artifact(args.output_artifact,
                                  type="model_export",
                                  description="Random Forest pipeline export")
    model_export.add_dir(model_save_path)
...
def plot_feature_importance(pipe, feat_names):
    ...
    feat_imp = pipe["random_forest"].regressor_.feature_importances_[: len(feat_names)-1]
    # For the NLP feature we sum across all the TF-IDF dimensions into a global
    # NLP importance
    nlp_importance = sum(pipe["random_forest"].regressor_.feature_importances_[len(feat_names) - 1:])
